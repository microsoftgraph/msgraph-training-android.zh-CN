...
首先扩展`GraphHelper`类以添加用于获取用户事件的函数。 打开**microsoft.windowsazure.activedirectory.graphhelper**文件, 并将以下`import`语句添加到文件顶部。
...
现在, `CalendarFragment`更新以使用这些新函数。 将以下`import`语句添加到**CalendarFragment**文件的顶部。
...
现在, 将以下函数添加到`CalendarFragment`类中, 以隐藏和显示进度栏, 以及在中`getEvents` `GraphHelper`为函数提供回调。
...
最后, 在`GraphHelper`类`onCreate`中重写函数以从 Microsoft Graph 中获取用户的事件。
...
现在, 您可以将 JSON 转储替换为以用户友好的方式显示结果的内容。 首先将 in**应用程序/res/layout/fragment_calendar**中的`TextView`替换替换为`ListView`。
...
接下来, 为中的`ListView`每个项目创建布局。 右键单击 "**应用/res/布局**" 文件夹, 然后选择 "**新建**", 然后选择 "**布局资源文件**"。 将文件`event_list_item`命名, 将**根元素**更改为`RelativeLayout`, 然后选择 **"确定"**。 打开 " **event_list_item** " 文件, 并将其内容替换为以下内容。
...
现在, `ListView`为创建自定义列表适配器。 必须为列表中的项目创建视图, 并将每个`Event`字段的字段映射到视图中的相应`TextView`字段。

右键单击**app/java/.com graphtutorial**文件夹, 然后依次选择 "**新建**"、" **java 类**"。 命名该类`EventListAdapter`并选择 **"确定"**。 打开 " **EventListAdapter** " 文件, 并将其内容替换为以下内容。
...
最后, 更新`CalendarFragment`类以使用`EventListAdapter`初始化。 `ListView` 打开**CalendarFragment**类, 并将以下函数添加到类中。
...
在`success`重写中的`mEventList = iEventCollectionPage.getCurrentPage();`行后添加以下代码行。

```java
addEventsToList();
```

运行应用程序, 登录, 然后点击 "**日历**" 导航项。 您应该会看到事件列表。
